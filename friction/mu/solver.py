...
class Solver():
    ...
    def NS_update(self, u_0, u_1, mu_2, phi_2, phi_1):
        ...
        def u_C_coef(bcs, index):
            return 4*R(bcs, index)[..., bm.newaxis]*dt*u_1(bcs, index)
        self.u_C.coef = u_C_coef
        self.u_C.clear()
        
        def u_SI_coef(bcs, index):
            result = R(bcs, index)[..., bm.newaxis]*(4*u_1(bcs, index) - u_0(bcs, index))
            result += 2*R(bcs, index)[..., bm.newaxis]*dt*bm.einsum('cqij, cqj->cqi', u_1.grad_value(bcs, index), u_0(bcs, index))
