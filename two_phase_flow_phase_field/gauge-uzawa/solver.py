...
class GaugeUzawaTwoPhaseFlowSolver:
    ...
    def viscoity(self, phi=None):
        """
        Update viscoity field according to ρ = 0.5(ρ₁ - ρ₂)φ + 0.5(ρ₁ + ρ₂), where φ is the phase-field variable.

        Parameters:
            phi(Function): Phase-field function, default is None.

        Returns:
            Function: viscoity function computed from phase-field.
        """
        if phi is None:
            phi = self.phi
        tag0 = phi[:] >1
    ...
    def lagrange_multiplier(self, A, b):
        """
        Calculate the Lagrange multiplier for the phase-field equation.
        """
        LagLinearForm = LinearForm(self.phispace)
        LagLinearForm.add_integrator(SourceIntegrator(source=1, q=self.q))
        ...
        b0 = bm.array([self.mesh.integral(self.pde.init_phi)])
    ...
    def solve_momentum(self, t1=0.0, phi0=None, uh0=None, phi=None, s=None):
        ...
        #visc_integrator = ViscousWorkIntegrator(coef=dt*mu, q=self.q)
        ...
        def momentum_source(bcs, index):
            ...
            result1 = dt * self.pde.mu_bar * s.grad_value(bcs, index)
        ...
        def f_source(bcs, index):
            ...
            #val[..., 0] = 0
            #val[..., 1] = -10
            #result = dt*rho1(bcs, index)[..., None] * val
            val = self.uspace.interpolate(cartesian(lambda p: self.pde.mom_force(p, t=0)))
            result = dt*rho1(bcs, index)[..., None] * val(bcs, index)
            return result
    ...
    def update_pressure(self, s1=None, ps1=None, phi=None):
        mu = self.viscoity(phi)
        ...
        def source_coef(bcs, index):
            ...
            result +=  self.pde.mu_bar * s1(bcs, index)
            return result
        lform.add_integrator(SourceIntegrator(source=source_coef, q=self.q))
        b = lform.assembly()
