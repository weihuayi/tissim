...
class GaugeUzawaTwoPhaseFlowSolver:
    ...
    def solve_phase_field(self, t1=0.0, mv=None, theta=0, phi0=None, uh0=None):
        """
        ...
        def source_coef(bcs, index):
            ...
            #添加移动网格项
            mesh_result = mv(bcs, index) - uh0(bcs, index)
            result += dt*bm.einsum('cqi, cqi->cq', result0, phi0.grad_value(bcs, index)))
    ...
    def solve_momentum(self, t1=0.0, mv=None, phi0=None, uh0=None, phi=None, s=None):
        """
        ...
        def mass_coef(bcs, index):
            ...
            # 添加移动网格项
            result -= dt*bm.einsum('cqi,cqi->cq', rho1.grad_value(bcs, index), mv(bcs, index))
            return result
        ...
        def conv_coef(bcs, index):
            ...
            result = dt * rhou
            # 添加移动网格项
            result -= dt * rho1(bcs, index)[..., None] * mv(bcs, index)
            return result
        ...
        def momentum_source(bcs, index):
            ...
            # 添加移动网格项
            mv_gardphi += bm.einsum('cqi,cqi->cq', phi1.grad_value(bcs, index), mv(bcs, index))
            result += dt*lam/gamma * mv_gardphi[..., None] * phi1.grad_value(bcs, index)
            return result
