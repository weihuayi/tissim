...
class two_phase_solver:
    ...
    def interface_refine(self, phi, u, p, n):
        ...
        for i in range(n):
            ...
            isMark = bm.mean(phic2f,axis=-1) > 0.2
            isMark = bm.logical_and(bm.mean(phic2f,axis=-1) < 0.8, isMark)
            isMark = bm.logical_and(bm.mean(phic2f,axis=-1) < 0.5 - 0.01,isMark)
            isMark = bm.logical_and(isMark,cellmeasure>4e-5)
            #isMark = bm.logical_and(isMark,cellmeasure>4e-6)
    ...
    def update_function(self, phi):
        ...
        rho[:] = self.pde.gas.rho/self.pde.liquid.rho + \
                (1 - self.pde.gas.rho/self.pde.liquid.rho) * phi[:]
        eta[:] = self.pde.gas.eta/self.pde.liquid.eta + \
                (1 - self.pde.gas.eta/self.pde.liquid.eta) * phi[:]
        return rho, eta
    ...
    def evo_BForm(self, u, phispace):
        ...
        Bform.add_integrator(ScalarDiffusionIntegrator(coef=dt*self.epsilon/5, q=self.q))
    ...
    def evo_LForm(self, u, phi0):
        ...
        def G_coef(bcs, index):
            result0 = dt*phi0(bcs, index) *(1-phi0(bcs, index))
            result1 = phi0.grad_value(bcs, index) / \
                    (1e-9+bm.linalg.norm(phi0.grad_value(bcs, index), axis=-1)[..., bm.newaxis])
            result = result0[..., bm.newaxis] * result1
            return result
        evo_GSI = GradSourceIntegrator(q=self.q)
        evo_GSI.source = G_coef
        Lform.add_integrator(evo_GSI)
        
        return Lform
    
    def rein_run(self, phi0):
        space = phi0.space
        mesh = space.mesh
        epsilon = bm.max(mesh.entity_measure('cell'))/3
        #epsilon = self.epsilon
        dtau = bm.max(mesh.entity_measure('cell'))/3
        maxit = 10
        tol = 1e-2

        rephi0 = space.function()
        rephi1 = space.function()
        rephi0[:] = phi0[:]
        
        for i in range(maxit): 
            A = self.reinit_BForm(space, dtau, epsilon).assembly()
            b = self.reinit_LForm(rephi0, dtau).assembly()
            rephi1[:] = spsolve(A, b, solver='mumps')

            error = mesh.error(rephi1, rephi0, power=1)
            eold = 1e10
            if eold < error or error< tol*dtau:
    ...
    def reinit_BForm(self, phispace, dtau, epsilon):
        Bform = BilinearForm(phispace)
        Bform.add_integrator(ScalarDiffusionIntegrator(coef=dtau*epsilon, q=self.q))
        Bform.add_integrator(ScalarMassIntegrator(q=self.q))
        return Bform
    
    def reinit_LForm(self, phi0, dtau):
        phispace = phi0.space
        Lform = LinearForm(phispace)
        
        @barycentric
        def source_coef(bcs, index=None):
            result = phi0(bcs, index) 
            return result
        
        rein_source = ScalarSourceIntegrator(q=self.q)
        rein_source.source = source_coef
        Lform.add_integrator(rein_source)
        
        def G_coef(bcs, index):
            result0 = dtau* phi0(bcs, index) * (1-phi0(bcs, index))
            result1 = phi0.grad_value(bcs, index) / \
                    (1e-9 + bm.linalg.norm(phi0.grad_value(bcs, index), axis=-1)[..., bm.newaxis])
            result = result0[..., bm.newaxis] * result1
            return result
        
        rein_G_source = ScalarSourceIntegrator(q=self.q)
        rein_G_source.source = G_coef
        Lform.add_integrator(rein_G_source)
        return Lform 
